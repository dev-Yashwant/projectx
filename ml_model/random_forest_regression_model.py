...
# Combine normalized numeric features with TF-IDF features
X_train_combined = np.hstack((X_train_normalized, tfidf_train.toarray()))
X_test_combined = np.hstack((X_test_normalized, tfidf_test.toarray()))
...
new_entry_normalized = scaler.transform([new_entry_features])
new_entry_comments = ' '.join(new_entry["comments"])
new_entry_tfidf = tfidf_vectorizer.transform([new_entry_comments])
new_entry_features_combined = np.hstack((new_entry_normalized, new_entry_tfidf.toarray().flatten()))  # Flatten TF-IDF array
